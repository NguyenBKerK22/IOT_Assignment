...
struct es_ws_02 {
    ...
    uint8_t rx_buffer[8]; // Buffer to hold the received data
    es_ws_02_data data; // Data structure to hold wind speed data
    CRC16 crc; // CRC object for error checking
    es_ws_02_data readSensor() { // Function to read the sensor data // Buffer to hold the data
        ...
        if (bytesRead == 7) { // Check if the response is valid
            data.windSpeed = (rx_buffer[3] << 8) | rx_buffer[4]; // Combine high and low bytes to get the pH value
